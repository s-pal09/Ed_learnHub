...
export function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser({ id: "", name: formData.name, email: formData.email, role: formData.role as UserRole })
      ...
      // Save pending signup data for the verify-otp route
      localStorage.setItem("learnhub-pending-signup", JSON.stringify(formData))
          ...
          {formData.password && (
            <ul className="mt-2 space-y-1">
              {passwordRequirements.map((req) => (
                <li
                  key={req.label}
                  className={`flex items-center gap-2 text-xs ${req.test(formData.password) ? "text-green-600" : "text-muted-foreground"}`}
                >
